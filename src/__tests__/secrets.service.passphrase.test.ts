...
describe('Passphrase-based key derivation (issue #20)', { timeout: 15000 }, () => {
  ...
  describe('encryptWithPassphrase / decryptWithPassphrase round trip', () => {
    ...
    it('should never store the raw passphrase or a derived key in the payload', async () => {
      ...
      const encrypted = await encryptWithPassphrase('some backup data', passphrase);
    ...
    it('should reject decryption with the wrong passphrase', async () => {
      const encrypted = await encryptWithPassphrase('vault backup contents', 'right-passphrase');
    ...
    it('should produce different ciphertext and salt for the same plaintext and passphrase (unique salt per encryption)', async () => {
      ...
      const first = JSON.parse(await encryptWithPassphrase(plaintext, passphrase));
      const second = JSON.parse(await encryptWithPassphrase(plaintext, passphrase));
    ...
    it('should reject a corrupted or malformed payload', async () => {
      await expect(decryptWithPassphrase('not-json', 'any-passphrase')).rejects.toThrow();
      await expect(
        decryptWithPassphrase(JSON.stringify({ version: 'unknown-version' }), 'any-passphrase')
      ).rejects.toThrow();
    });
  });
});