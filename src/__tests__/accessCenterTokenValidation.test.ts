...
describe("AccessCenter Network-Aware Token Validation", () => {
  ...
  describe("Stellar Soroban Token Pass Ownership Check", () => {
    it("should verify token pass ownership for Stellar vaults via Soroban storage", async () => {
      ...
      await stellarService.mintAccessToken(
        vaultId,
        userAccount,
        "ipfs://QmStellarMetadata101"
      );

      // Query token ownership specifically on Stellar network
      const hasToken = await contractService.hasVaultToken(
        userAccount,
        vaultId,
        "stellar"
      );
  ...
  describe("Non-Owner Access Rejection", () => {
    it("should return false for accounts that do not hold any access token", async () => {
      ...
      const stellarHasToken = await contractService.hasVaultToken(
        nonOwner,
        vaultId,
        "stellar"
      );
      const evmHasToken = await contractService.hasVaultToken(
        nonOwner,
        vaultId,
        "avalanche"
      );
  ...
  describe("Ownership Query Failure & Edge Case Handling", () => {
    it("should return false and not grant access when account or vaultId are invalid", async () => {
      expect(await contractService.hasVaultToken("", 101, "stellar")).toBe(
        false
      );
      expect(
        await contractService.hasVaultToken(userAccount, 0, "stellar")
      ).toBe(false);
      expect(
        await contractService.hasVaultToken(userAccount, -1, "stellar")
      ).toBe(false);
    });

    it("should produce a clear false failure state when Soroban token query throws error", async () => {
      const vaultId = 401;
      vi.spyOn(stellarService, "hasVaultToken").mockRejectedValueOnce(
        new Error("RPC Connection Failure")
      );
  ...
  describe("Matching Host Network Selection", () => {
    it("should route token pass query to the matching host network specified by parameter", async () => {
      ...
      const stellarSpy = vi
        .spyOn(stellarService, "hasVaultToken")
        .mockResolvedValue(true);

      const resultStellar = await contractService.hasVaultToken(
        userAccount,
        vaultId,
        "stellar"
      );
